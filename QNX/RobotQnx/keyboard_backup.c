...
//	flushall();	
	...
	while(killFlg == 0){
		...
		kbInput = kb_getc();
			// Could have use the case statement here
		if( kbInput == 'q' || kbInput == 'Q'){
			// quit program and reset all robot motors
// send message to GUIport
			writeMsg(kb_simCoid,GUIQUIT);
			readMsg(kb_simCoid);
// perform all the necessary house keeping here! including killing the other 2 GUI processes
...
// Set up message body and send it to GUIport
			...
			flushall();
			writeMsg(kb_simCoid,SHOULDERUP);
			readMsg(kb_simCoid);
			timer_settime(timerid,0, &timer,NULL);
			while(pulse_count<1)
			{
			...
			flushall();
			writeMsg(kb_simCoid,SHOULDERDW);
			readMsg(kb_simCoid);
			timer_settime(timerid,0, &timer,NULL);
			while(pulse_count<1)
			{
			...
			flushall();
			writeMsg(kb_simCoid,GRIPPERCLOSE);
			readMsg(kb_simCoid);
			timer_settime(timerid,0, &timer,NULL);
			while(pulse_count<1)
			{
			...
			flushall();
			writeMsg(kb_simCoid,CONVEYORON);
			readMsg(kb_simCoid);
			...
			flushall();
			writeMsg(kb_simCoid,CONVEYOROFF);
			readMsg(kb_simCoid);
			...
			flushall();
			writeMsg(kb_simCoid,UNLOADLEFT);
			readMsg(kb_simCoid);
			delay(10);
			writeMsg(kb_simCoid,ESTOP);
			...
			flushall();
			writeMsg(kb_simCoid,UNLOADRIGHT);
			readMsg(kb_simCoid);
			delay(10);
			writeMsg(kb_simCoid,ESTOP);
